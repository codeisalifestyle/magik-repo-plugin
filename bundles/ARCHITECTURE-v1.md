...
4. **Close the agentic loop** — orient → strategize → implement → verify → (HITL if taste) → ship → **KB sync** → clean up. Taste/creative work requires human approval (often drafts); programmatically verifiable work does not. After merge, update durable KB entries. Resolve code and vault/KB merge conflicts in-loop. See `rules/agentic-work.mdc` / skill `agentic-e2e-loop`.
