...
4. **Close the agentic loop.** Orient → strategize → implement → verify → (HITL if taste) → ship → **KB sync** → clean up. Taste/creative work gets a human approval gate (often drafts); programmatically verifiable work does not. Ship includes **CI watch + fix-forward**, then merge. After merge, sync durable KB entries. Resolve **code and vault/KB merge conflicts in-loop**. **Multi-machine synchronicity**: orient on start (`git pull --ff-only` if behind), cloud-save on pause (WIP commit `git commit -m "wip: <checkpoint> [skip ci]"` + push to origin), and never use global `git stash` across worktrees. Cleanup: tear down ephemeral worktrees; reset the primary checkout to the integration branch. See skill `agentic-e2e-loop` / rule `agentic-work`.
...
🔁 **Agentic E2E loop** — every substantive task closes: verify locally, human-in-the-loop for taste/creative work (drafts + approval), ship with **CI watch + fix-forward**, **sync the knowledge base**, then clean up (worktree teardown or primary reset to `develop`/`main`). Same loop in the IDE and in worktrees — not tied to a specific orchestrator.
